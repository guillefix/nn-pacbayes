...
d["oversampling2"].unique()
d["test_sensitivity"].unique()
d["test_acc"].unique()
...
plt.savefig("img/acc_sigmab_all_marginal_"+dataset+".png")
...
for L in range(1,6):
    ...
    # d1[(d1["centering"]) & (d1["number_layers"]==L)][["sigmab","test_sensitivity"]].plot.line("sigmab","test_sensitivity", legend=False)
    plt.xlabel("$\\sigma_b$")
    plt.ylabel("Accuracy")
    plt.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
