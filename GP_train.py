...
def main(_):
    ...
    for init in tasks:
        ...
        if not doing_regression:
            th = 0.5
            train_loss, train_acc = 0, 1.0
            test_loss, test_acc = sum([cross_entropy_loss(x,preds[i]) for i,x in enumerate(test_ys)])/len(test_ys), sum([(preds[i]>th)==x for i,x in enumerate(test_ys)])/len(test_ys)
        else:
            train_acc = train_loss = 0
            test_acc = test_loss = sum([cross_entropy_loss(x,preds[i]) for i,x in enumerate(test_ys)])/len(test_ys)
        ...
        if not ignore_non_fit or train_acc == 1.0:
            ...
            function = preds[:test_function_size]>0.5
