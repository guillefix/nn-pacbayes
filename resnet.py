...
def ResNet(input_shape=None, classes=10, block='bottleneck', residual_unit='v2', repetitions=None,
           ...
           initial_pooling='max', final_pooling=None, top='classification',transition_strides=None):
    ...
    for i, r in enumerate(repetitions):
        ...
        if transition_strides == None:
            transition_strides_block = [(1, 1)] * r
            if transition_dilation_rate == (1, 1):
                transition_strides_block[0] = (2, 2)
        else:
            transition_strides_block = [ transition_strides[i] ]*r
        block = _residual_block(block_fn, filters=filters,
                                stage=i, blocks=r,
                                is_first_layer=(i == 0),
                                dropout=dropout,
                                transition_dilation_rates=transition_dilation_rates,
                                transition_strides=transition_strides_block,
                                dilation_rates = [(1,1)]*r,
